...
if __name__=="__main__":
    ...
    #     wlst.write(word_lst)
    # wlst.close()
    with codecs.open("result/english_wordlist.txt",'r',encoding='utf-8') as f:    
       word_lst = f.read()
    f.close()
    tags = jieba.analyse.extract_tags(word_lst,topK = 500, withWeight = True, allowPOS=allow_pos)
    # tags = jieba.analyse.extract_tags(word_lst,topK = 50, withWeight = True)
    # print tags
    word_dict= {}
    weight_scale = 1000000
    with open("result/less/wordCount_en.txt",'w') as wf2: 
        for item in tags:
        ...
        color_mask = imread('image/songer3.jpg')
        wordcloud = WordCloud(
        ...
        plt.axis('off')
        plt.show()
        fig.savefig('image/english_huahua.png',pad = 0, bbox_inches = 'tight')
        plt.close()
    ...
    wbk.save('result/less/wordCount_en.xls') 
